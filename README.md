...
- Queue + Events: публикация события создания комментария через Celery task.
- Cache: кэш списка комментариев и CAPTCHA с graceful fallback при ошибках Redis.
...
3. Для запуска без Redis установите:
   - `REDIS_URL=`
...
## Примечания по Redis
- Если `REDIS_URL` задан, используются Redis cache + Redis channel layer.
- Если `REDIS_URL` пустой, автоматически включаются `LocMemCache` и `InMemoryChannelLayer`.
- Ошибки Redis в cache-операциях не роняют HTTP-запросы.
