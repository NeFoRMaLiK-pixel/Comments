...
## Stack
- Backend: Django 5, Django ORM, DRF
- DB: PostgreSQL (по умолчанию в Docker), SQLite fallback локально
- Frontend: Vue 3 (SPA без перезагрузки страницы)
- Queue: Celery + Redis
- Cache: Redis cache
- Events: Django signals + `CommentEvent` table
...
## Junior+ дополнения
- `Queue`: асинхронная обработка события создания комментария в Celery task.
- `Cache`: кэширование выдачи списка комментариев по сортировке и странице.
- `Events`: событие `comment.created` сохраняется в таблицу `CommentEvent`.
- `JWT`: `/api/auth/token/` и `/api/auth/token/refresh/`.
...
## Примечания по сдаче
- Код готов к размещению в Git-репозитории.
- Docker-конфигурация приложена.
- Схема БД в `db/schema.sql` приложена.
- Видео-демо нужно записать отдельно после деплоя (по вашему чеклисту).
